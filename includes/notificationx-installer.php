...
class Nx_Installer {
    ...
    public function init_hooks() {
        ...
        add_action( 'wp_ajax_Nx_Installer_installer', array( $this, 'install_wpsp_core' ) );
    }
    /**
    ...
    public function activation_notice() {
        ...
        <script type="text/javascript">
            jQuery(document).ready( function($) {
                $('#nx-installer-btn').on('click', function (e) {
                    var self = $(this);
                    e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        data: {
                            action: 'Nx_Installer_installer',
                            _wpnonce: '<?php echo wp_create_nonce('Nx_Installer_installer'); ?>'
                        },
    ...
    public function install_wpsp_core() {
        check_ajax_referer( 'Nx_Installer_installer' );

        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( __( 'You don\'t have permission to install the plugins' ) );
        }

        $wpsp_status = $this->install_plugin( 'notificationx', 'notificationx.php' );
